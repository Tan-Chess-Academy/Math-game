...
def draw_calc_lesson(surf):
    ...
    pygame.draw.circle(gs2,(*col,7),(W*3//4,H//2),360)
    surf.blit(gs2,(0,0))

    # Left: portrait + bio
    px2,py2=30,50
    draw_calc_portrait(surf,teacher,px2+44,py2+56,size=48)
    ns3=CF_BIG.render(teacher["name"],True,col)
    surf.blit(ns3,(px2,py2+114))
    es3=CF_XS.render(teacher["era"],True,(100,100,140))
    surf.blit(es3,(px2,py2+150))
    pygame.draw.line(surf,(30,30,50),(px2,py2+170),(px2+360,py2+170),1)
    for gi,gline in enumerate([teacher["greeting"],teacher["greeting2"]]):
        gs3=CF_SM.render(gline,True,(160,160,180))
        surf.blit(gs3,(px2,py2+178+gi*24))

    # Graph
    draw_calc_graph(surf,teacher,px2,py2+236,360)

    # Right: lesson
    lx2=W//2-10; ly2=44; lw2=W-lx2-16
    pygame.draw.rect(surf,tuple(int(c*0.2) for c in col),(lx2,ly2,lw2,44),border_radius=8)
    pygame.draw.rect(surf,col,(lx2,ly2,lw2,44),2,border_radius=8)
    tl2=CF_MED.render(lesson["title"],True,col)
    surf.blit(tl2,(lx2+lw2//2-tl2.get_width()//2,ly2+10))
    ...
    for li2,line2 in enumerate(calc_fill_text(lesson["text"])):
        ...
        ls4=CF_SM.render(visible2,True,(210,220,240))
        surf.blit(ls4,(lx2+8,ly2+58+li2*30))

    # Lesson dots
    nl2=len(teacher["lessons"])
    dot_y2=H-62
    pygame.draw.line(surf,(25,25,45),(lx2,dot_y2-6),(lx2+lw2,dot_y2-6),1)
    for di2 in range(nl2):
        dc2=col if di2==calc_mode["lesson_idx"]%nl2 else (35,35,55)
        pygame.draw.circle(surf,dc2,(lx2+14+di2*20,dot_y2+6),6,0 if di2==calc_mode["lesson_idx"]%nl2 else 2)

    # Nav hint
    nh2=CF_XS.render(
        f"Lesson {calc_mode['lesson_idx']%nl2+1}/{nl2}  |  TAB next  "
        f"|  ← x-slider  →  |  +/- slices  |  K/ESC exit  |  T: Trig",
        True,(55,55,80))
    surf.blit(nh2,(lx2+40,dot_y2+2))
...
def draw_gallery_lesson(surf):
    ...
    # ── Left: portrait + bio ──────────────────────────────
    px, py = 40, 60
    draw_math_portrait(surf, m, px+50, py+60, size=52)

    ns = GF_BIG.render(m["name"], True, col)
    surf.blit(ns, (px, py+120))
    es = GF_XS.render(m["era"] + "  ·  " + m["country"], True, (100,100,140))
    surf.blit(es, (px, py+154))

    # Greeting
    pygame.draw.line(surf, (30,30,50), (px, py+174), (px+340, py+174), 1)
    for gi, gline in enumerate([m["greeting"], m["greeting2"]]):
        gs2 = GF_SM.render(gline, True, (160,160,180))
        surf.blit(gs2, (px, py+182+gi*24))

    # Boss unlocked by
    boss_names = "  +  ".join(
        NUMBER_BOSSES.get(bid,{}).get("name","?")
        for bid in m["boss_ids"])
    bl = GF_XS.render("Unlocked by defeating: " + boss_names, True, (70,100,70))
    surf.blit(bl, (px, py+236))
    ...
    # Lesson title bar
    pygame.draw.rect(surf, tuple(int(c*0.2) for c in col),
                     (lx, ly, lw, 44), border_radius=8)
    pygame.draw.rect(surf, col, (lx, ly, lw, 44), 2, border_radius=8)
    tl = GF_MED.render(lesson["title"], True, col)
    surf.blit(tl, (lx+lw//2-tl.get_width()//2, ly+10))
    ...
    for li, line in enumerate(lesson["lines"]):
        ...
        ls = GF_SM.render(visible, True, (210, 220, 240))
        surf.blit(ls, (lx+10, ly+58+li*32))

    # Lesson dots navigation
    nl = len(m["lessons"])
    dot_y = H - 70
    pygame.draw.line(surf, (25,25,45), (lx, dot_y-8), (lx+lw, dot_y-8), 1)
    for di in range(nl):
        dc = col if di == gallery_lesson % nl else (35,35,55)
        pygame.draw.circle(surf, dc, (lx+16+di*22, dot_y+6), 7,
                           0 if di==gallery_lesson%nl else 2)
    nh = GF_XS.render(
        f"Lesson {gallery_lesson%nl+1}/{nl}  |  TAB next  |  ← → browse  |  G/ESC close",
        True, (55,55,80))
