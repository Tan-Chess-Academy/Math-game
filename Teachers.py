...
def evaluate():
    ...
    # Iota substitution: replace standalone i / ι / iota with (1j)
    # Must be careful not to replace 'i' inside words like 'math'
    import re as _re
    # Replace 'iota', 'ι', then lone 'i' not adjacent to letters
    raw = raw.replace("iota", "(1j)")
    raw = raw.replace("ι",    "(1j)")
    # Lone 'i' — preceded/followed by non-alpha (number, space, operator, paren)
    raw = _re.sub(r'(?<![a-zA-Z_])i(?![a-zA-Z_0-9])', '(1j)', raw)

    # Also allow 'j' directly (Python native imaginary literal)
    # e.g. "2j", "3+4j" already work in eval
...
def draw_calc_graph(surf, teacher, lx, ly, lw):
    """Draw an interactive mini-graph relevant to the current teacher."""
    gw, gh = lw-10, 180
    gx, gy = lx+5, ly
    pygame.draw.rect(surf,(8,8,18),(gx,gy,gw,gh),border_radius=8)
    pygame.draw.rect(surf,teacher["color"],(gx,gy,gw,gh),2,border_radius=8)

    cx2 = gx+gw//2; cy2 = gy+gh//2
    xrange = 3.0; yrange = 4.0
    # Axes
    pygame.draw.line(surf,(40,40,60),(gx+4,cy2),(gx+gw-4,cy2),1)
    pygame.draw.line(surf,(40,40,60),(cx2,gy+4),(cx2,gy+gh-4),1)
    ...
    # Draw function curve
    prev = None
    for px2 in range(gx+4, gx+gw-4):
        xv = (px2-cx2)/gw*xrange*2
        try:
            fv = _calc_eval(fn, xv)
            py2 = int(cy2 - fv/yrange*(gh//2-8))
            py2 = max(gy+4, min(gy+gh-4, py2))
            if prev:
                pygame.draw.line(surf,(100,200,160),prev,(px2,py2),2)
            prev = (px2,py2)
        except Exception:
            prev = None

    # Show derivative tangent line at x_val
    if focus == "deriv":
        xv = calc_mode["x_val"]
        h2 = 0.001
        try:
            ...
            px3 = int(cx2 + xv/xrange*gw//2)
            py3 = int(cy2 - fv/yrange*(gh//2-8))
            # Tangent line
            for dx2 in range(-40,41):
                tx2 = px3+dx2
                ty2 = int(py3 - slope*dx2/gw*xrange*2/yrange*(gh//2-8))
                if gx+4<=tx2<=gx+gw-4 and gy+4<=ty2<=gy+gh-4:
                    surf.set_at((tx2,ty2),teacher["color"])
            pygame.draw.circle(surf,teacher["color"],(px3,py3),5)
        except Exception: pass
        # Label
        ls = CF_TINY.render(f"f'({xv:.1f})={calc_live('chain_val')}", True, teacher["color"])
        surf.blit(ls,(gx+8,gy+gh-18))

    # Riemann sum rectangles
    elif focus in ("integ","series"):
        n = calc_mode["n_slices"]
        x0v, x1v = 0.0, calc_mode["x_val"] if calc_mode["x_val"]>0.1 else 1.0
        for i in range(n):
            xv = x0v + (x1v-x0v)*i/n
            try:
                fv = _calc_eval(fn,xv)
                rx = int(cx2 + xv/xrange*gw//2)
                rw2 = max(1,int((x1v-x0v)/n/xrange*gw//2*2))
                ry2 = int(cy2 - fv/yrange*(gh//2-8))
                pygame.draw.rect(surf,(*teacher["color"],80),(rx,min(cy2,ry2),rw2,abs(cy2-ry2)))
                pygame.draw.rect(surf,teacher["color"],(rx,min(cy2,ry2),rw2,abs(cy2-ry2)),1)
            except Exception: pass
        ls = CF_TINY.render(f"n={n} strips, area≈{calc_live('riemann_approx')}", True, teacher["color"])
        surf.blit(ls,(gx+8,gy+gh-18))

    # Controls hint
    ch = CF_TINY.render("← →: x-slider   +/-: slices   F: function", True,(50,50,70))
    surf.blit(ch,(gx+4,gy+gh+4))

def draw_calc_teacher_select(surf):
    surf.fill((4,4,14))
    for row in range(0,H,3):
        frac=row/H
        pygame.draw.line(surf,(int(4+6*frac),int(4+4*frac),int(14+20*frac)),(0,row),(W,row))

    t2 = CF_BIG.render("CALCULUS  TEACHERS", True, (0,220,180))
    surf.blit(t2,(W//2-t2.get_width()//2,22))
    s2 = CF_XS.render("All main bosses defeated — welcome to the realm of calculus!", True,(80,80,120))
    surf.blit(s2,(W//2-s2.get_width()//2,66))

    n2 = len(CALC_TEACHERS)
    card_w = min(195,(W-40)//(n2)-8)
    card_h = 230
    total_w = n2*(card_w+10)-10
    sx2 = W//2-total_w//2

    for i,teacher in enumerate(CALC_TEACHERS):
        cx2 = sx2+i*(card_w+10); cy2=88
        sel = (i==calc_mode["selected"]%n2)
        col = teacher["color"]
        bg2 = (20,20,36) if not sel else (28,28,54)
        pygame.draw.rect(surf,bg2,(cx2,cy2,card_w,card_h),border_radius=12)
        pygame.draw.rect(surf,col if sel else (35,35,55),(cx2,cy2,card_w,card_h),3 if sel else 1,border_radius=12)
        draw_calc_portrait(surf,teacher,cx2+card_w//2,cy2+60,size=38)
        ns2=CF_SM.render(teacher["name"],True,col if sel else (150,150,170))
        if ns2.get_width()>card_w-8: ns2=CF_XS.render(teacher["name"],True,col if sel else (150,150,170))
        surf.blit(ns2,(cx2+card_w//2-ns2.get_width()//2,cy2+108))
        es2=CF_TINY.render(teacher["era"],True,(80,80,110))
        surf.blit(es2,(cx2+card_w//2-es2.get_width()//2,cy2+126))
        # Focus badge
        fb=CF_TINY.render(teacher["focus"].upper(),True,col)
        surf.blit(fb,(cx2+card_w//2-fb.get_width()//2,cy2+142))
        g2=CF_TINY.render(teacher["greeting"][:card_w//7],True,(55,55,80))
        surf.blit(g2,(cx2+4,cy2+160))
        if sel:
            p2=int(3*math.sin(pygame.time.get_ticks()*0.005))
            pygame.draw.rect(surf,col,(cx2-p2,cy2-p2,card_w+p2*2,card_h+p2*2),2,border_radius=14)
            en=CF_XS.render("ENTER to learn",True,col)
            surf.blit(en,(cx2+card_w//2-en.get_width()//2,cy2+card_h+8))

    # Mode switch hint
    ms=CF_XS.render("K: Calculus mode   T: Switch to Trig mode   ESC: Exit",True,(55,55,80))
...
def draw_calc_lesson(surf):
    ...
    # Subtle bg glow on right half only
    gs2 = pygame.Surface((W, H), pygame.SRCALPHA)
    pygame.draw.circle(gs2, (*col, 6), (W*3//4, H//2), 340)
    surf.blit(gs2, (0, 0))

    # ── Left column: portrait + bio + graph (max 420px wide) ───
    LEFT_W = 420
    px2, py2 = 24, 44

    # Portrait
    draw_calc_portrait(surf, teacher, px2+44, py2+52, size=44)

    # Name (clamp width)
    ns3 = CF_BIG.render(teacher["name"], True, col)
    if ns3.get_width() > LEFT_W - 8:
        ns3 = CF_MED.render(teacher["name"], True, col)
    surf.blit(ns3, (px2, py2+106))

    es3 = CF_XS.render(teacher["era"], True, (100,100,140))
    surf.blit(es3, (px2, py2+140))

    pygame.draw.line(surf, (30,30,50), (px2, py2+160), (px2+LEFT_W-20, py2+160), 1)

    # Greeting — clip to LEFT_W
    for gi, gline in enumerate([teacher["greeting"], teacher["greeting2"]]):
        gs3 = CF_SM.render(gline, True, (150,160,175))
        # Clip surface to left column width
        clip = min(gs3.get_width(), LEFT_W - 10)
        surf.blit(gs3, (px2, py2+168+gi*22), (0, 0, clip, gs3.get_height()))

    # Graph
    draw_calc_graph(surf, teacher, px2, py2+220, LEFT_W - 10)

    # ── Right column: lesson panel ──────────────────────────
    DIVIDER = LEFT_W + 30
    lx2 = DIVIDER
    ly2 = 36
    lw2 = W - lx2 - 16
    ...
    pygame.draw.rect(surf, tuple(int(c*0.22) for c in col),
                     (lx2, ly2, lw2, 46), border_radius=8)
    pygame.draw.rect(surf, col, (lx2, ly2, lw2, 46), 2, border_radius=8)
    tl2 = CF_MED.render(lesson["title"], True, col)
    surf.blit(tl2, (lx2+lw2//2 - tl2.get_width()//2, ly2+11))

    # Lesson text (typewriter, word-wrapped)
    calc_mode["text_t"] = min(calc_mode["text_t"] + 1.2, 9999)
    total_chars2 = 0
    line_y = ly2 + 60
    for li2, line2 in enumerate(calc_fill_text(lesson["text"])):
        total_chars2 += len(line2)
        visible2 = (line2 if calc_mode["text_t"] >= total_chars2
                    else line2[:max(0, len(line2)-(total_chars2-int(calc_mode["text_t"])))])
        ls4 = CF_SM.render(visible2, True, (210, 222, 240))
        surf.blit(ls4, (lx2+10, line_y))
        line_y += 32

    # Divider line
    nl2   = len(teacher["lessons"])
    dot_y = H - 58
    pygame.draw.line(surf, (28,28,48), (lx2, dot_y-8), (lx2+lw2, dot_y-8), 1)

    # Lesson dots
    for di2 in range(nl2):
        dc2 = col if di2 == calc_mode["lesson_idx"] % nl2 else (38,38,58)
        pygame.draw.circle(surf, dc2, (lx2+16+di2*22, dot_y+7), 7,
                           0 if di2 == calc_mode["lesson_idx"]%nl2 else 2)

    # Nav hint
    nh2 = CF_XS.render(
        f"Lesson {calc_mode['lesson_idx']%nl2+1}/{nl2}  |  TAB next  |  B: Quiz  "
        f"|  <- -> x-slider  |  +/- slices  |  F cycle fn  |  K/ESC exit",
        True, (55,55,82))
    surf.blit(nh2, (lx2+50, dot_y+2))
...
def handle_calc_events(event):
    ...
    if phase=="teacher_select":
        ...
        if k==pygame.K_b:
            ...
            calc_mode["n_slices"]=min(50,calc_mode["n_slices"]+2)
        elif k==pygame.K_MINUS or event.unicode=="-":
            calc_mode["n_slices"]=max(2,calc_mode["n_slices"]-2)
        elif k==pygame.K_f:
